...
export default function LinkCard({link, refreshLinks}) {
    ...
    const archiveLink = async () => {
        link.archived = true;
        try{
            await fetch('/api/updateLink', {method:'PUT',body: JSON.stringify(link)});
            refreshLinks();
        } catch(error){
            console.error("BZZZZT", error);
        }
    }
    ...
    const deleteLink = async () => {
        const id = link._id;
        try{
            await fetch('/api/deleteLink', {method:'DELETE',body: JSON.stringify({id})});
            refreshLinks();
        } catch(error){
            console.error("BZZZZT", error);
        }
